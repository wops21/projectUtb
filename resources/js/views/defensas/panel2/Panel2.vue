  ...
  <div>
    <v-container>
      <div>
        ...
        <div class="text-h5">programaciones segundo panel</div>
      </div>
      <v-simple-table>
                              ...
                              <v-layout pt-3>
                                <v-flex xs4>
                                  ...
                                  <div   v-for="item3 in item.relacion_estudiante.relacion_solicitud">
                                    <v-icon color="red" @click="generarInforme(item)" v-show="itemChild.nroCartaSolInforme == 0">error</v-icon>
                                      <div class="caption"><v-icon color="green" v-show="itemChild.nroCartaSolInforme !== 0" @click="generateCartaInforme(itemChild, item, item3)">print</v-icon></div>
                                ...
                                <v-flex xs5>
                                  ...
                                  <div v-bind:style="itemChild.fechaLimiteInforme >  timestamp ? ' backgroundColor: #6BF660 '
                    : 'backgroundColor:#FE714A' ">{{ itemChild.fechaLimiteInforme }}</div>
                                </v-flex>
                              </v-layout>
                            </v-timeline-item>
                          </v-timeline>
    ...
    <v-dialog v-model="dialogInformeView" max-width="900">
      <v-card>
        <v-card-title class="blue darken-2 white--text">Solicitud de informe de correción</v-card-title>
        <v-card-text>
          <v-container>
...
export default {
  ...
  methods: {
    ...
    loadPanel2: async function () {
      try {
        const response = await panel1Service.loadDefensasPanel2(this.comboData,
    ...
    loadSearch: async function () {
      const response = await panel1Service.loadDefensasPanel2Search(
        this.comboData,
        this.buscador
      );
      this.panels1 = response.data.data;
    ...
    loadEstudiantes: async function () {
      try {
        const response = await estudianteService.getEstudiantesP2(this.carreraSelect);
  ...
  await panel1Service.habilitarPanel3(this.aprobarData.id, formData)
  this.loadPanel2();
  ...
  this.$swal({
    ...
    deshabilitar: async function(item){
      ...
      try {
        if (result.isConfirmed) {
          await panel1Service.deshabilitarPanel3(item.id);
    ...
    editAsignacion(asignacion) {
      ...
      this.editAsignaciondData = { ...asignacion };
    },
    changeDate(date) {
