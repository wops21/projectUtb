<template>
  <div>
    <v-container>
      <div>
        <v-row>
          ...
          <v-spacer></v-spacer>
          <v-col cols="12" sm="1">
            <v-btn dark @click="showNewProgramacionDialog()"
              ><v-icon>assignment_add</v-icon></v-btn
            >
          </v-col>
        </v-row>
        <div class="text-h5">programaciones primer panel</div>
      </div>
      <v-simple-table>
        <template v-slot:default>
          <thead>
            <tr>
              ...
              <th>Hora defensa</th>
              <th>Fecha defensa</th>
              <th>Habilitacion 2do panel</th>
              <th>Acciones</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in panels1" :key="index">
              <td>{{ index + 1 }}</td>
              <td>
                <v-expansion-panels>
                  <v-expansion-panel>
                    <v-expansion-panel-header>
                      ...
                      <template v-slot:actions>
                        <div v-for="itemC in item.relacion_detalle_estudiante">
                          <v-icon color="error" v-if="itemC.esCorrecion == 'pendiente'">
                            mdi-alert-circle
                          </v-icon>
                          <v-icon
                          color="teal"
                              ...
                              >
                                <v-layout pt-12>
                                  <v-flex xs10>
                                    ...
                                    <div class="caption">
                                      <v-icon @click="completarCorrecion(itemC)"
                                        >check_small</v-icon
                                      >
                                    </div>
                                    <div
                                      class="caption"
                                      v-bind:style="
                                        itemC.fechaLimiteCorrecion > timestamp
                                          ? ' backgroundColor: #6BF660 '
                                          : 'backgroundColor:#FE714A'
                                      "
                                    >
                                      Limite de correcion {{ itemC.fechaLimiteCorrecion }}
                                    </div>
                                  </v-flex>
                                </v-layout>
                              </v-timeline-item>
              ...
              <td v-for="itemChild in item.relacion_detalle">
                <v-expansion-panels>
                  <v-expansion-panel>
                    <v-expansion-panel-header>
                      ...
                      <v-card light flat>
                        <v-card-text class="py-0">
                          <v-timeline align-top dense>
                                ...
                                <v-flex xs4>
                                  ...
                                  >
                                    <div class="caption">
                                      <v-icon
                                        color="red"
                                        @click="generCartaDef(item)"
                                        v-show="itemChild.nroCartaDefensa == 0"
                                        >error</v-icon
                                      >
                                      <v-icon
                                        color="green"
                                        v-show="itemChild.nroCartaDefensa !== 0"
                                        @click="generarCarta(itemChild, item, item2)"
                                        >print</v-icon
                                      >
                                    </div>
                                  </div>
                              ...
                              <v-layout pt-3>
                                <v-flex xs4>
                                  ...
                                  >
                                    ...
                                    <div class="caption">Limite de acta:</div>
                                  </div>
                                </v-flex>
                                <v-flex xs5 v-if="itemChild.actaRespuesta == 'pendiente'">
                                  ...
                                  <div
                                    class="caption"
                                    v-bind:style="
                                      itemChild.fechaLimiteActa > timestamp
                                        ? ' backgroundColor: #6BF660 '
                                        : 'backgroundColor:#FE714A'
                                    "
                                  >
                            ...
                            <v-timeline-item
                              v-show="itemChild.solicitudInforme == 'pendiente'"
                              color="pink"
                              small
                            >
                              <v-layout pt-3>
                                <v-flex xs4>
                                  <strong>Generar</strong>
                                  <div class="caption">INFORMES</div>
                                  <div
                                    v-for="item3 in item.relacion_estudiante
                                      .relacion_solicitud"
                                  >
                                    <v-icon
                                      color="red"
                                      @click="generarInforme(item)"
                                      v-show="itemChild.nroCartaSolInforme == 0"
                                      >error</v-icon
                                    >
                                    <div class="caption">
                                      <v-icon
                                ...
                                <v-flex xs5>
                                  ...
                                  <div
                                    v-bind:style="
                                      itemChild.fechaLimiteInforme > timestamp
                                        ? ' backgroundColor: #6BF660 '
                                        : 'backgroundColor:#FE714A'
                                    "
                                  >
              ...
              <td class="text-center">
                <v-icon @click="habilitarData(item)" color="red">check_box</v-icon>
              </td>
              <td>
      ...
      <v-card>
        ...
        >
          <v-card-title class="blue darken-2 white--text"
            >Crear Programación</v-card-title
          >
          <v-card-text>
            <v-container>
              <v-row>
              ...
              <v-row>
                ...
                <v-col cols="12" sm="6" md="6">
                  ...
                  >
                    ...
                    <v-date-picker v-model="date" format="yyyy/MM/dd"  scrollable>
                      <v-spacer></v-spacer>
          ...
          <v-card-actions>
            ...
            <v-btn @click="hideNewProgramacionDialog" color="blue-grey" dark>Cancelar</v-btn>
            <v-btn type="submit" :disabled="!valid" color="success" class="mr-4" dark>Guardar</v-btn>
          </v-card-actions>
        </v-form>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="dialogInformeView" max-width="900">
      <v-card>
        <v-card-title class="blue darken-2 white--text"
          >Solicitud de informe de correción</v-card-title
        >
        <v-card-text>
          <v-container>
            <div class="text-center"><GenerarInforme /></div>
            <PlantillaInforme />
          </v-container>
        </v-card-text>
      </v-card>
    ...
    <v-dialog v-model="dialogAprobar" max-width="300">
      <v-card class="container">
        ...
        >
          ...
          <v-card-text>
            <v-text-field
              v-model="aprobarData.notaDefensa"
              label="Ingrese la nota"
              :rules="[
                (v) => !!v || 'Debe ingresar la nota del postulante',
                (v) => /[0-9]$/.test(v) || 'Debe ingresar un nombre valido',
              ]"
              required
            >
            </v-text-field>
...
import GenerarInforme from "./GenerarInforme.vue";
...
export default {
  ...
  data() {
    return {
      correciones: [
        { correcion: "Con correciones", bCorrecion: "si" },
        { correcion: "Sin correciones", bCorrecion: "no" },
      ],
  ...
  methods: {
    ...
    loadSearch: async function () {
      const response = await panel1Service.loadDefensasPanel1Search(
        this.comboData,
        this.buscador
      );
      this.panels1 = response.data.data;
    ...
    loadEstudiantes: async function () {
      try {
        const response = await estudianteService.getEstudiantesP1(this.carreraSelect);
    ...
    createProgramacion: async function () {
      if (this.$refs.form.validate() === false) {
        ...
        const formattedDate = this.$moment(this.programacionData.fechaDefensa).format('YYYY/MM/DD');
        ...
        formData.append("fechaDefensa", formattedDate);
        ...
        try {
          ...
          this.hideNewProgramacionDialog();
          ...
          switch (error.response.status) {
            case 401:
              this.$swal({
                icon: "error",
                title: "Oops...",
                text: "El postulante no cuenta con jurados, verifique por favor",
              });
              break;
              case 402:
              this.$swal({
                icon: "error",
                title: "Oops...",
                text: "El postulante no cuenta con tutor, verifique por favor",
              });
              break;
            case 403:
              this.$swal({
    ...
    habilitar: async function () {
      if (this.$refs.form.validate() === false) {
        ...
        try {
          ...
          await panel1Service.habilitarPanel2(this.aprobarData.id, formData);
          this.loadPanel1();
          this.loadEstudiantes();
          ...
          switch (error.response.status) {
            case 402:
              this.$swal({
                ...
                title: "Oops...",
                text:
                  "Debe aprobar la entrega de los informes de habilitacion para el segundo panel",
              });
              break;
            case 403:
              this.$swal({
    ...
    deshabilitar: async function (item) {
      Swal.fire({
        ...
        try {
          if (result.isConfirmed) {
            await panel1Service.deshabilitarPanel2(item.id);
    ...
    generCartaDef: async function (item) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          await panel1Service.generarCorrelativoCartaDef(item.id);
          this.loadPanel1();
          Swal.fire("Confirmado!", "Se ha generado su carta con exito.", "success");
        }
      });
    },
    generarInforme: async function (item) {
      Swal.fire({
        title: "Esta seguro de generar la carta de defensa?",
        text: "¡No podrás revertir esto!",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          await panel1Service.generarCorrelativoInforme(item.id);
          this.loadPanel1();
          Swal.fire("Confirmado!", "Se ha generado su carta con exito.", "success");
        }
      });
    ...
    generateCartaInforme(itemChild, item, item3) {
      ...
      this.changeDate(item);
      this.changeTodayInforme(itemChild);
    ...
    showNewProgramacionDialog() {
      this.GET_CARRERAS();
      this.loadEstudiantes();
      this.dialogCreate = true;
    },
    hideNewProgramacionDialog() {
